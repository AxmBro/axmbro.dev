...
export const ProjectsSearchbar = ({ search, setSearch, buttonsActionsArray }: ProjectsSearchbarProps) => {
  ...
  return (
    <div className={styles.searchBarWrapper}>
            ...
            {buttonsActionsArray.map((buttonAction, index) => {
              return (
                <Button
                  ...
                  buttonColor={index === 0 ? "blue" : "default"}
                />
              )
            })}
          </Popover>
      ...
      {search && (<div onClick={() => setSearch("")}>
        <Button style={{ width: "2.75rem", minWidth: "0" }} text="X" buttonColor={"defaultEmpty2"}></Button>
      </div>)}
    </div>
  )
}